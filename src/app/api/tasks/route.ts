...
export async function POST(request: Request) { try { const userId = await requireClerkUserId(); const input = validateTask(await request.json()); return NextResponse.json(await tasksRepository.create(userId, { title: input.title, description: input.description || '', completed: input.completed ?? false, archived: input.archived ?? false, priority: input.priority || 'medium', dueDate: input.dueDate || '', dueTime: input.dueTime || '', projectId: input.projectId || '', categoryId: input.categoryId || '', tagIds: input.tagIds || [], estimatedDuration: input.estimatedDuration ?? 0, completedAt: input.completedAt || '', notes: input.notes || '' }), { status: 201 }) } catch (error) { return failure(error) } }
